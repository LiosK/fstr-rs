...
//!
//! let c_str = buffer.slice_up_to('\0');
//! assert_eq!(c_str, "&#x40;");
//!
//! use core::fmt::Write as _;
//! write!(buffer.writer_at(c_str.len()), " COMMERCIAL AT")?;
//! assert_eq!(buffer.slice_up_to('\0'), "&#x40; COMMERCIAL AT");
...
impl<const N: usize> FStr<N> {
    ...
    pub const fn from_str_lossy(s: &str, filler: u8) -> Self {
        ...
        // (truncated at a char boundary, if applicable) and trailing ASCII filler bytes
    ...
    /// assert_eq!(x.slice_up_to('\n'), "quick brown fox");
    /// assert_eq!(x.slice_up_to('🦊'), "quick brown fox\n");
    /// # assert_eq!(FStr::from_bytes([])?.slice_up_to(' '), "");
    /// # Ok::<_, core::str::Utf8Error>(())
    /// ```
    pub fn slice_up_to(&self, terminator: char) -> &str {
    ...
    pub fn from_fmt(args: fmt::Arguments<'_>, filler: u8) -> Result<Self, fmt::Error> {
        ...
        if fmt::Write::write_fmt(&mut w, args).is_ok() {
            w.fill_and_finish(filler);
            // SAFETY: ok because the entire array has been initialized with valid string slices and
            // trailing ASCII filler bytes
...
impl<const N: usize> fmt::Debug for FStr<N> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct(match fstr!(32; "FStr<{}>", N) {
            Ok(ref buffer) => buffer.slice_up_to('\0'),
