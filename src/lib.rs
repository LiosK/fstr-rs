...
impl<const N: usize> FStr<N> {
    ...
    /// assert_eq!(x.slice_until(' '), "quick");
    /// assert_eq!(x.slice_until('w'), "quick bro");
    /// assert_eq!(x.slice_until('\n'), "quick brown fox");
    /// assert_eq!(x.slice_until('🦊'), "quick brown fox\n");
    /// # assert_eq!(FStr::from_bytes([])?.slice_until(' '), "");
    /// # Ok::<_, core::str::Utf8Error>(())
    /// ```
    pub fn slice_until(&self, terminator: char) -> &str {
    ...
    /// assert_eq!(x.slice_until('\0'), "  0042  ");
...
impl<const N: usize> FStr<N> {
    ...
    /// A deprecated synonym for [`FStr::slice_until`].
    // #[deprecated(since = "0.2.20", note = "renamed to `slice_until`")]
    pub fn slice_to_terminator(&self, terminator: char) -> &str {
        self.slice_until(terminator)
    }
...
impl<const N: usize> fmt::Debug for FStr<N> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct(match fstr!(32; "FStr<{}>", N) {
            Ok(ref buffer) => buffer.slice_until('\0'),
